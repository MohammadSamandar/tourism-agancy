...
def add_product_to_order(request):
    ...
    if request.user.is_authenticated:
        product = Tour.objects.filter(id=product_id, is_active=True, is_delete=False).first()
...
class PaymentsList(ListView):
    model = Order
    template_name = 'order_module/payments_list.html'
...
def payments_detail(request: HttpRequest, payment_id):
    ...
    return render(request, 'order_module/payments_detail.html', context)
